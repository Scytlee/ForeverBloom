...
namespace ForeverBloom.WebApi.UnitTests.Helpers;

public sealed class ProblemDetailsHelperTests
{
    [Theory]
    ...
    [Fact]
    public void CreateBadRequestProblemDetails_ShouldBuild400BadRequestEnvelope()
    {
        const int statusCode = 400;
        var errors = new[]
        {
            new ErrorDetail { Code = "Name.Required", Message = "Name is required" },
            new ErrorDetail { Code = "Email.Invalid", Message = "Email format is invalid" }
        };

        var result = ProblemDetailsHelper.CreateBadRequestProblemDetails(statusCode, errors);
    ...
    [Fact]
    public void CreateBadRequestProblemDetails_ShouldBuild422UnprocessableEntityEnvelope()
    {
        const int statusCode = 422;
        var errors = new[]
        {
            new ErrorDetail { Code = "Quantity.Invalid", Message = "Quantity must be positive" }
        };
