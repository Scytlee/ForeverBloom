...
public sealed class ArchiveCategoryCommandTests
{
    private static Result<ArchiveCategoryCommand> CreateCommandWith(
        long categoryId = 1,
        uint rowVersion = 1)
    {
        return ArchiveCategoryCommand.Create(categoryId, rowVersion);
    }
    ...
    public void Create_ShouldCorrectlyConstructCommand_WhenAllInputIsValid()
    {
        ...
        var createResult = ArchiveCategoryCommand.Create(categoryId, rowVersion);
