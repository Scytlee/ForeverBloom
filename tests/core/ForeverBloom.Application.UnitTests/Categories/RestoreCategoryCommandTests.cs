...
public sealed class RestoreCategoryCommandTests
{
    private static Result<RestoreCategoryCommand> CreateCommandWith(
        long categoryId = 1,
        uint rowVersion = 1)
    {
        return RestoreCategoryCommand.Create(categoryId, rowVersion);
    }
    ...
    public void Create_ShouldCorrectlyConstructCommand_WhenAllInputIsValid()
    {
        ...
        var createResult = RestoreCategoryCommand.Create(categoryId, rowVersion);
