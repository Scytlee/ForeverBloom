...
public sealed class ArchiveProductCommandTests
{
    private static Result<ArchiveProductCommand> CreateCommandWith(
        long productId = 1,
        uint rowVersion = 1)
    {
        return ArchiveProductCommand.Create(
            productId,
            rowVersion);
    }
    ...
    public void Create_ShouldCorrectlyConstructCommand_WhenAllInputIsValid()
    {
        ...
        var createResult = ArchiveProductCommand.Create(
            productId,
            rowVersion);
