...
public class SitemapPageModel : PageModel
{
    ...
    private void ProcessCategoriesResponse(ApiResponse<GetCategoriesSitemapDataResponse> categoriesResponse, XmlWriter xmlWriter, string baseUrl)
    {
        if (categoriesResponse.IsSuccess)
        {
            foreach (var category in categoriesResponse.Value!.Items)
            {
                AddUrlElement(xmlWriter, baseUrl, $"/kategoria/{category.Slug}", category.UpdatedOn);
            }
        }
        else
        {
    ...
    private void ProcessProductsResponse(ApiResponse<GetProductsSitemapDataResponse> productsResponse, XmlWriter xmlWriter, string baseUrl)
    {
        if (productsResponse.IsSuccess)
        {
            foreach (var product in productsResponse.Value!.Items)
            {
                AddUrlElement(xmlWriter, baseUrl, $"/produkt/{product.Slug}", product.UpdatedOn);
            }
        }
        else
        {
